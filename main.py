...
parser.add_argument('vallist', type=str,
                    help='path to val list file')
parser.add_argument('-a', '--arch', default='resnetd50')
...
parser.add_argument('--gpu', default=None, type=int,
                    help='GPU id to use.')
parser.add_argument('--label_smooth_eta', default=None, type=float,
                    help='GPU id to use.')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        model = getattr(model_zoo, args.arch)(pretrained=True)
    else:
        print("=> creating model '{}'".format(args.arch))
        model = getattr(model_zoo,args.arch)(zero_init_residual=True)
    ...
    if args.label_smooth_eta is None:
        criterion = nn.CrossEntropyLoss().cuda(args.gpu)
    else:
        criterion = LSCritierion(args.label_smooth_eta).cuda(args.gpu)
    ...
    val_loader = torch.utils.data.DataLoader(
        loader(args.vallist, transforms.Compose([
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        acc1, acc5 = mixup_accuracy(output.data, target_a, target_b, lam, topk=(1, 5))
...
def adjust_learning_rate(optimizer, epoch, args):
    """Sets the learning rate by warmup and cosine decay"""
    if epoch < args.warmup:
        lr = args.lr * (epoch + 1) / args.warmup
    else:
        total_epoch_exclude_warmup = args.epochs - args.warmup
        current_epoch_exclude_warmup = epoch - args.warmup
        lr = args.lr * 0.5 * (1 + np.cos(np.pi * current_epoch_exclude_warmup / total_epoch_exclude_warmup))
